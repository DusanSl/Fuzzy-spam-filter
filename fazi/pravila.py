...
def kontroler_spam_score(mu: dict) -> np.ndarray:
    ...
    p03 = min(kljucne["zastupljene"], linkovi["minimalni"])
    aktivacije.append(np.fmin(p03, score_sumnjiv))
    p04 = min(np.fmax(kljucne["zanemarljive"], kljucne["zastupljene"]), linkovi["umereni"])
    aktivacije.append(np.fmin(p04, score_sumnjiv))
    p05 = min(kljucne["dominantne"], caps["uobicajen"])
    aktivacije.append(np.fmin(p05, score_sumnjiv))


    p06 = np.fmax(kljucne["dominantne"], linkovi["brojni"])
    aktivacije.append(np.fmin(p06, score_spam))
    p07 = min(kljucne["zastupljene"], np.fmax(caps["agresivan"], inter["agresivna"]))
    aktivacije.append(np.fmin(p07, score_spam))
    p08 = min(linkovi["umereni"], np.fmax(caps["agresivan"], inter["agresivna"]))
    aktivacije.append(np.fmin(p08, score_spam))
    p09 = kljucne["dominantne"]
    aktivacije.append(np.fmin(p09, score_spam))
